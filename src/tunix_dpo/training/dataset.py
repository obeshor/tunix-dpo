...
class HHRLHFDataset:
    ...
    def _pack(self, prompt_ids: list[int], response_ids: list[int]) -> np.ndarray:
        """Pack prompt + response into a fixed-length array of token IDs.

        Returns int64 because the embedding layer's backward pass requires
        Long indices — passing int32 forward-passes fine but explodes during
        loss.backward() with "Embedding indices are expected to be of scalar
        type Long".
        """
    ...
    def _mask(self, prompt_ids: list[int], response_ids: list[int]) -> np.ndarray:
        ...
        Kept as int32: masks are multiplied with logits, never used as indices,
        so the narrower dtype saves memory on long sequences.
        """
