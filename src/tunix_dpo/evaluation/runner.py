...
def run_toxigen(
    ...
    summary = {
        ...
        "disc_true_positive_rate": float(np.mean([r["disc_correct"] for r in toxic_rows]))
        if toxic_rows
        else 0.0,
        "disc_true_negative_rate": float(np.mean([r["disc_correct"] for r in benign_rows]))
        if benign_rows
        else 0.0,
        "groups": groups,
        "per_row": per_row,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.skip_toxigen:
        tox_base = run_toxigen(
            args.base_model, "base", out, args.device, args.max_per_group, args.max_new_tokens
        )
        tox_tuned = run_toxigen(
            args.tuned_model, "tunix_dpo", out, args.device, args.max_per_group, args.max_new_tokens
        )
