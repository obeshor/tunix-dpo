...
def compare_truthfulqa(base: dict, tuned: dict) -> dict:
    ...
    comp["bootstrap"] = {
        ...
        "base":  {"mean": round(float(np.mean(base_bin)),  4), "ci_95": list(map(lambda x: round(x, 4), bci))},
        "tuned": {"mean": round(float(np.mean(tuned_bin)), 4), "ci_95": list(map(lambda x: round(x, 4), tci))},
...
def compare_toxigen(base: dict, tuned: dict) -> dict:
    ...
    comp["bootstrap"] = {
        ...
        "base":  {"mean": round(float(np.mean(base_scores)),  4), "ci_95": list(map(lambda x: round(x, 4), bci))},
        "tuned": {"mean": round(float(np.mean(tuned_scores)), 4), "ci_95": list(map(lambda x: round(x, 4), tci))},
